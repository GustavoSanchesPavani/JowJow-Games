...
// Função para renderizar o elemento de produto na página principal
        ...
        <form action=\"index.php\" method=\"post\">
            <div class=\"card shadow rounded-0\">
                ...
                <div class=\"card-body rounded-0\">
                    ...
                    <h5>
                        " . ($product_details['prev_price'] > 0 ? "<small><s class=\"text-secondary\">R$ " . (number_format($product_details['prev_price'], 2)) . "</s></small>" : "") . "
                        <span class=\"price\">R$ " . (number_format($product_details['current_price'], 2)) . "</span>
                    </h5>

                    <button type=\"submit\" class=\"btn btn-primary my-3 rounded-0\" name=\"add\"><i class=\"fa fa-cart-plus\"> Adicionar ao carrinho</i></button>
...
function cartItems($product_details)
{
    ...
    // Verifica se a chave está definida e, se não, define o valor padrão como zero
        ...
        <div class=\"border rounded\">
            <div class=\"row bg-white\">
                ...
                <div class=\"col-md-6 py-3\">
                    ...
                    <small class=\"text-secondary\">Descrição: {$product_details['description']}</small>
                    <h5 class=\"pt-2\">R$ {$product_details['current_price']}</h5>
                    <button onclick=\"if(confirm('Você tem certeza que deseja remover o produto do carrinho?') === true)location.replace('cart.php?action=removeItem&id={$product_details['id']}');\" type=\"button\" class=\"btn btn-outline-danger btn-sm rounded-0 mx-2\" name=\"remove\"><i class=\"fas fa-trash\"></i> Remover item</button>
                </div>
